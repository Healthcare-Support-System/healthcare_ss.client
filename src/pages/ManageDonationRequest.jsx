...
const ManageDonationRequests = () => {
  ...
  useEffect(() => { fetchDonationRequests(); }, []);
  ...
  const handleAccept = async (requestId) => {
    ...
    try {
      ...
      setRequests((prev) => prev.map((r) =>
        r._id === requestId || r.id === requestId ? { ...r, ...updatedRequest, status: "accepted" } : r
      ));
  ...
  const handleReject = async (requestId) => {
    ...
    try {
      ...
      const updatedRequest = response.data.data;
      setRequests((prev) => prev.map((r) =>
        r._id === requestId || r.id === requestId ? { ...r, ...updatedRequest, status: "rejected" } : r
      ));
  ...
  /* ── Status config using new palette ── */
  const statusConfig = {
    pending:  { bg: "bg-[#FDF4E7]", text: "text-amber-700",   border: "border-amber-200",   dot: "bg-amber-400",   label: "Pending"  },
    accepted: { bg: "bg-[#E8F5EF]", text: "text-emerald-700", border: "border-emerald-200", dot: "bg-emerald-500", label: "Accepted" },
    rejected: { bg: "bg-[#FDF0F1]", text: "text-rose-600",    border: "border-rose-200",    dot: "bg-rose-400",    label: "Rejected" },
  };

  const InfoField = ({ label, value }) => (
    <div className="flex flex-col gap-0.5">
      <span className="text-[9.5px] font-bold uppercase tracking-[0.11em] text-[#C9686B]">{label}</span>
      <span className="text-[12.5px] font-medium text-[#2e2840] truncate">{value || "N/A"}</span>
    </div>
  );
  ...
  if (loading) {
    return (
      <>
        ...
        `}</style>
        <div className="bg-[#FFF9F5] min-h-screen p-5 flex items-center gap-3 text-[13px] font-medium text-[#4A3F7A]"
          style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <span style={{ width:16, height:16, border:"2px solid #E2CDD3", borderTopColor:"#4A3F7A",
            borderRadius:"50%", animation:"sp .75s linear infinite", flexShrink:0, display:"inline-block" }} />
    ...
    <>
      ...
      {popup.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background: "rgba(46,40,64,0.35)", backdropFilter: "blur(3px)" }}>
          <div className="bg-white rounded-2xl shadow-2xl border border-[#E2CDD3] w-full max-w-sm overflow-hidden">
            <div className="flex items-start gap-3 px-5 pt-5 pb-3">
              <div className="w-10 h-10 rounded-xl bg-[#FDF0F1] border border-rose-100 flex items-center justify-center flex-shrink-0">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D4737A"
              ...
              <div className="pt-0.5">
                <h3 className="text-[14px] font-bold text-[#2e2840] leading-tight">{popup.title}</h3>
                <p className="text-[12.5px] text-[#9A90A8] mt-1.5 leading-relaxed">{popup.message}</p>
              </div>
            </div>
              ...
              {popup.type === "confirm" ? (
                <>
                  <button onClick={closePopup}
                    className="px-4 py-2 text-[12.5px] font-semibold text-[#5a5070] border border-[#E2CDD3]
                      rounded-xl bg-white hover:bg-[#F7EBF0] transition-all duration-150 active:scale-95">
                    Cancel
                  </button>
                ...
                <button onClick={closePopup}
                  className="px-4 py-2 text-[12.5px] font-semibold text-white bg-[#4A3F7A]
                    hover:bg-[#C9686B] rounded-xl transition-all duration-150 active:scale-95 shadow-sm">
                  OK
                </button>
              )}
            </div>
      ...
      <div className="mdr bg-[#FFF9F5] min-h-screen p-5">
        ...
        {successMessage && (
          <div className="flex items-start gap-2 bg-[#E8F5EF] border border-emerald-200 border-l-4 border-l-emerald-500
        ...
        {requests.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-[12px] text-[#9A90A8]">New requests will appear here once submitted.</p>
          </div>
        )}
                ...
                <div key={id}
                  ...
                  <div className="mdr-card-top flex items-center justify-between px-4 py-3 border-b border-[#F0E5E8]">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#C9686B]">Request ID</span>
                        <span className="text-[11.5px] font-semibold text-[#2e2840] truncate max-w-[220px]">{id}</span>
                      </div>
                    </div>

                    {/* Status badge */}
                    <span className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-3 py-1 rounded-full border ${sc.bg} ${sc.text} ${sc.border}`}>
                  ...
                  <div className="px-4 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-4">
                      ...
                      <InfoField label="Created At"         value={request.created_at
                        ? new Date(request.created_at).toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" })
                        : "N/A"} />
                      {request.reference_code && (
                        <InfoField label="Reference Code" value={request.reference_code} />
                      )}
                      {request.accepted_at && (
                        <InfoField label="Accepted At" value={new Date(request.accepted_at).toLocaleString()} />
                      )}
                    </div>

                    {/* Message */}
                    {request.message && (
                      <div className="bg-[#FDFAF8] border border-[#F0E5E8] rounded-xl px-3.5 py-3 mb-4">
                        <span className="text-[9.5px] font-bold uppercase tracking-[0.11em] text-[#C9686B] block mb-1.5">
                          Donor Message
                        </span>
                        <p className="text-[12.5px] text-[#2e2840] leading-relaxed">{request.message}</p>
                      </div>
                    )}

                    {/* ── Pending actions ── */}
                    {status === "pending" && (
                      <div className="flex items-center gap-2.5 pt-0.5 border-t border-[#F0E5E8] mt-1 pt-3">
                        <select
                          value={status}
                          onChange={(e) => handleStatusChange(id, e.target.value)}
                          className="mdr-sel text-[12px] font-semibold px-3 py-1.5 rounded-lg
                            border border-[#E2CDD3] bg-[#FDF4E7] text-amber-700
